...
class SemiMatch(nn.Module):
    ...
    def forward(self, 
        ...
        if self.args.refined_corr_filtering == 'mutual':
            ...
            grid_x, grid_y = self.soft_argmax(corr_weak_S_Tvec)
            pred_flow_weak_S_Tvec = torch.cat((grid_x, grid_y), dim=1)
            pred_map_weak_S_Tvec = self.convert_unNormFlow_to_unNormMap(pred_flow_weak_S_Tvec)
            pred_map_weak_S_Tvec = pred_map_weak_S_Tvec.int()

        # entropy
        uncertainty = (-(corr_weak_S_Tvec_transformed_prob+1e-6) * torch.log(corr_weak_S_Tvec_transformed_prob+1e-6)).sum(dim=1)
        index_weak_not_T = index_weak_S_Tvec.detach().clone()
        index_weak = index_weak_S_Tvec_transformed.detach().clone()

        #mask calcucation#
        #score_mask#
        self.args.p_cutoff = 0.7
        score_mask = score_weak_S_Tvec.ge(self.args.p_cutoff)
        #resize src bbox size aligned to map size
        bbox = mini_batch['trg_bbox'].cuda()
        x1, y1, x2, y2 = torch.round(bbox[:,0] / W).long(),\
                         torch.round(bbox[:,1] / H).long(),\
                         torch.round(bbox[:,2] / W).long(),\
                         torch.round(bbox[:,3] / H).long()
        #trg_mask#
        trg_bbox_mask = (x_S_Tvec >= x1.repeat_interleave(H*W).view(-1, H, W)) & \
                        (x_S_Tvec <= x2.repeat_interleave(H*W).view(-1, H, W)) & \
                        (y_S_Tvec >= y1.repeat_interleave(H*W).view(-1, H, W)) & \
                        (y_S_Tvec <= y2.repeat_interleave(H*W).view(-1, H, W)) 
        ...
        #src_mask#
        bbox = mini_batch['src_bbox'].cuda() # B x (x1, y1, x2, y2)
        x1, y1, x2, y2 = torch.round(bbox[:,0] / W).long(),\
                         torch.round(bbox[:,1] / H).long(),\
                         torch.round(bbox[:,2] / W).long(),\
                         torch.round(bbox[:,3] / H).long()
        ...
        src_bbox_mask = (xx >= x1.repeat_interleave(H*W).view(-1, H, W)) & \
        ...
        if self.args.use_fbcheck_mask:
            ...
            mask2D_tgt = score_mask & trg_bbox_mask & occ_S_Tvec.squeeze(1).bool()
        else:
            mask2D_tgt = score_mask & trg_bbox_mask

        mask2D_not_T = src_bbox_mask & mask2D_tgt
        ...
        mask2D_tgt_transformed = torch.round(self.transform_by_grid(mask2D_tgt.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
                                        interpolation_mode=self.args.interpolation_mode)).squeeze().bool()
        mask2D = src_bbox_mask & mask2D_tgt_transformed
        ...
        x_s, y_s = (index_strong % W), torch.div(index_strong, W, rounding_mode='floor')  
        ...
        if it % 2 == 1:
            if mask2D[0].sum() == 0:
                ...
                mask_src_kp2D_weak_not_T = (mask2D_not_T[0] == True).nonzero(as_tuple=False)
                mask_src_kp2D_weak_not_T = mask_src_kp2D_weak_not_T.transpose(-1,-2)
                #type1 (by_softArgmax)
                ...
                #type2 (by_Argmax)
                mask_tgt_1D = index_weak_not_T[0][mask2D_not_T[0]]
                mask_tgt_kp2D_weak_not_T = torch.cat(((mask_tgt_1D % W).view(1,-1), (mask_tgt_1D // W).view(1,-1)), dim=0)
                # Visualization Variable(weak_T)#        
                # mask_src_kp2D_weak = (mask_2D[0] == True).nonzero(as_tuple=False).transpose(-1, -2)
                mask_src_kp2D_weak = (mask2D[0] == True).nonzero(as_tuple=False).transpose(-1, -2)
                mask_tgt_1D = index_weak[0][mask2D[0]]
                mask_tgt_kp2D_weak = torch.cat(((mask_tgt_1D % W).view(1,-1), (mask_tgt_1D // W).view(1,-1)), dim=0)
                
                # Visualization Variable(strong_T)        
                mask_tgt_kp2D_strong = (mask2D[0] == True).nonzero(as_tuple=False).transpose(-1, -2)
                mask_src_1D = index_strong[0][mask2D[0]]
                ...
                # diff_point = 0.0
                diff_idx = []
                # for idx, (weak_pt, strong_pt) in enumerate(zip(mask_src_kp2D_weak.permute(-1,-2), mask_src_kp2D_strong.permute(-1,-2))):  # weak_pt strong_pt (x,y)
        ...
        if self.args.loss_mode == 'contrastive':
            #margin
            mask_2D = mask2D & strong_margin_mask
            B, Svec, T_h, T_w = corr_strong.size()
        ...
        diff_ratio = (~(index_weak[mask_2D] == index_strong[mask_2D])).sum() / masked_num  # 
        ...
        #     Loss = loss_sup + loss_unsup + loss_self + diff_ratio
