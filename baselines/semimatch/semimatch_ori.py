...
class SemiMatch(nn.Module):
    ...
    def forward(self, 
        ...
        index_weak_not_T = index_weak.detach().clone()
        ...
        xy = torch.cat((x.unsqueeze(1).float(),y.unsqueeze(1).float()),1)
        ...
        mask_2D = torch.round(self.transform_by_grid(mask2D_not_T.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
                                        interpolation_mode=self.args.interpolation_mode)).squeeze().bool()
        # plus margin_mask
        # mask_2D = mask_2D & transformed_margin_mask
        ...
        if it % 40 == 1:
            if mask_2D[0].sum() == 0:
                ...
                mask_src_1D = index_weak_not_T[0][mask2D_not_T[0]]
        ...
        if mode == 'CATs' or mode == 'ncnet':
            ...
            return loss_unsup, loss_self, diff_ratio