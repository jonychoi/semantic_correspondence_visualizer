...
class SemiMatch(nn.Module):
    ...
    def forward(self, 
        ...
        # corr_weak_new = corr_weak.reshape(B, W, H, H, W).permute(0,2,1,3,4)
        # corr_weak_S_Tvec = corr_weak_new.permute(0,3,4,1,2)            
        ...
        if self.args.refined_corr_filtering == 'mutual':
            ...
            # x, y = (index_weak % W), torch.div(index_weak, W, rounding_mode='floor')y_S_Tvec
            xy = torch.zeros_like(pred_map_weak_gt)
            x_S_Tvec, y_S_Tvec = (index_weak_S_Tvec % W), (index_weak_S_Tvec // W)

            print("x:", x[0])
            print("X_S_Tvec:", x_S_Tvec[0])

            # unNormMap1D_to_NormMap2D
        # elif self.args.refined_corr_filtering == 'soft_argmax':
            ...
            x_S_Tvec, y_S_Tvec = pred_map_weak_S_Tvec[:,0,:,:], pred_map_weak_S_Tvec[:,1,:,:]

        # entropy
        uncertainty = (-(corr_weak_S_Tvec_prob+1e-6) * torch.log(corr_weak_S_Tvec_prob+1e-6)).sum(dim=1)
        ...
        if self.args.use_fbcheck_mask:
            ...
            mask2D_not_T = mask2D_not_T & occ_S_Tvec.squeeze(1).bool()
        ...
        if it % 2 == 1:
            if mask_2D[0].sum() == 0:
                ...
                #transpose_coord_like (x,y)
                # mask2D_not_T = src_bbox_mask & trg_bbox_mask
                # mask2D_not_T = score_mask
                ...
                mask_tgt_1D = index_weak[0][mask_2D[0]]
        ...
        if self.args.loss_mode == 'contrastive':
            #margin
            mask_2D = mask_2D & strong_margin_mask
            B, Svec, T_h, T_w = corr_strong.size()
