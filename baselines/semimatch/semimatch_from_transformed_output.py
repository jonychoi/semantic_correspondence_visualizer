...
class SemiMatch(nn.Module):
    ...
    def forward(self, 
        ...
        if self.args.refined_corr_filtering == 'mutual':
            ...
            index_weak_T_Svec = pred_map_weak[:,1,:,:].int()*W + pred_map_weak[:,0,:,:]
            index_weak_T_Svec = torch.round(index_weak_T_Svec).clamp(0, 15)
        ...
        x_transformed, y_transformed = torch.round(self.transform_by_grid(x_T_Svec.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
                              interpolation_mode=self.args.interpolation_mode)).squeeze().long().clamp(0, 15), \
                                       torch.round(self.transform_by_grid(y_T_Svec.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
                              interpolation_mode=self.args.interpolation_mode)).squeeze().long().clamp(0, 15)
        index_weak_transformed = y_transformed * W + x_transformed
        index_weak_transformed = index_weak_transformed.detach().clone()
        mask2D_transformed = torch.round(self.transform_by_grid(mask2D.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
                                        interpolation_mode=self.args.interpolation_mode)).squeeze().bool()
        uncertainty_transformed = self.transform_by_grid(uncertainty.unsqueeze(1).float(), mini_batch[self.args.aug_mode].to(self.device), mode=self.args.aug_mode,
