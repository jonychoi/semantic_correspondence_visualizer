...
from confidence_estimator.con_estimator_tf_not_residual import con_estimator_tf_not_residual_shallow_mlp
from confidence_estimator.con_estimator_tf_not_residual import con_estimator_tf_not_residual_shallow_proj_mlp
from confidence_estimator.con_estimator_tf_residual import con_estimator_tf_residual_shallow
...
from confmatch.utils import parse_list, load_checkpoint, load_checkpoint_resume, load_checkpoint_conf, save_checkpoint, boolean_string
...
if __name__ == "__main__":
    ...
    #confmatch#
    parser.add_argument('--confmatch_resume', type=boolean_string, nargs='?', const=True, default=False)
    parser.add_argument('--conf_est_resume', type=boolean_string, nargs='?', const=True, default=False)
    ...
    parser.add_argument('--cut_off',help="threshold for confidence.",default=0.50, type=float)
    parser.add_argument('--con_est_depth', type=int, default=4)
    parser.add_argument('--con_est_input_map_type', type=str, default='norm', choices=['norm', 'unNorm'])    
    parser.add_argument('--con_est_input_map_direction', type=str, default='forward', choices=['forward', 'reverse'])
    parser.add_argument('--con_est_input_mode', type=str, default='tgt', choices=['src', 'tgt', 'symmetric'])
    ...
    #tf_sh
    parser.add_argument('--feat', type=str, default = 'mlp')
    parser.add_argument('--corr', type=str, default = 'mlp')
    parser.add_argument('--map', type=str, default = 'mlp')
    ...
    elif args.con_est_type == 'tf_not_residual_shallow':
    ...
    if args.pretrained:
        #to continue model
        if args.confmatch_resume:
            model, optimizer, optimizer_con, scheduler, scheduler_con, start_epoch, best_val = load_checkpoint_resume(confmatch,
                                                                                                            optimizer,
                                                                                                            optimizer_con,
                                                                                                            scheduler, 
                                                                                                            scheduler_con,
                                                                                                            filename = args.pretrained)
        elif args.conf_est_resume:                                                                                                
            model, _, _, _, _, _, best_val = load_checkpoint_conf(confmatch,
                                                                optimizer,
                                                                optimizer_con,
                                                                scheduler, 
                                                                scheduler_con,
    ...
    args.save_path = args.cur_snapshot
    ...
    if args.data_parallel:
        confmatch = nn.DataParallel(confmatch.to(device))
    else:
        confmatch = confmatch.to(device)        

    train_started = time.time()
    result_file = '{}_results.txt'.format(args.time_stamp)
    result_file_all = '{}_results_all.txt'.format(args.time_stamp)
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss = optimize_confmatch.train_epoch(confmatch,
                                                    model,
...
##validate##
        ...
        if args.benchmark == 'pfpascal':
            val_dataset = download.load_dataset('pfpascal', args.datapath, 'img', device, 'test', args.augmentation, args.feature_size)
            val_dataloader = DataLoader(val_dataset,
                        batch_size=args.batch_size,
                        num_workers=args.n_threads,
                        shuffle=True)             
            dict_name = 'P'
            val_dictionary = {'P_0.05':0, 'P_0.1':0, 'P_0.15':0, 'W_0.05':0, 'W_0.1':0, 'W_0.15':0}
            thres_list = [0.05, 0.1, 0.15]
            for i in range(len(thres_list)):
                Evaluator.initialize('pfpascal', thres_list[i])
                val_loss_grid, val_mean_pck = optimize_confmatch.validate_epoch(confmatch,
                                                            val_dataloader,
                                                            device,
                                                            epoch=epoch)
                
                
                val_key_name = '{}_{}'.format(dict_name, thres_list[i])
                val_dictionary[val_key_name] = val_mean_pck

            
            dict_name = 'W'    
            thres_list = [0.05, 0.1, 0.15]
            val_dataset = download.load_dataset('pfwillow', args.datapath, 'bbox', device, 'test', args.augmentation, args.feature_size)
            val_dataloader = DataLoader(val_dataset,
                        batch_size=args.batch_size,
                        num_workers=args.n_threads,
                        shuffle=True)                
            for i in range(len(thres_list)):
                ...
                Evaluator.initialize('pfwillow', thres_list[i])
                val_loss_grid, val_mean_pck = optimize_confmatch.validate_epoch(confmatch,
                                                            val_dataloader,
                                                            device,
                                                            epoch=epoch)
            ...
            val_mean_pck = val_dictionary['P_0.1']    
            ...
            with open(os.path.join(save_path, result_file_all),'a+') as file:
                file.write(f'{val_dictionary}\n')
        ...
        if epoch % 10 == 0 :
            save_checkpoint({'epoch': epoch + 1,
                            ...
                            'scheduler': scheduler.state_dict(),
                            'best_loss': best_val},
                        True, save_path, 'epoch_{}.pth'.format(epoch + 1))
        ...
        save_checkpoint({'epoch': epoch + 1,
                        ...
                        'scheduler_con':scheduler_con.state_dict(),                            
                        'best_loss': best_val},
                        True, save_path, 'latest_model.pth')
