...
#refined_corr 을 쓴다
class con_estimator_tf(nn.Module):
    ...
    def forward(self, corr, source, target):
        ...
        x1 = torch.cat((x0.transpose(-1, -2), target.mean(1).unsqueeze(1)), dim=3) + pos_embed #torch.Size([32, 1, 256, 384])
        x2 = self.proj(self.blocks(x1)).transpose(-1, -2) + corr  # swapping the axis for swapping self-attention. # x2 : torch.Size([32, 8, 256, 256])

        x3 = torch.cat((x2, source.mean(1).unsqueeze(1)), dim=3) + pos_embed # x3 :torch.Size([32, 1, 256, 384])
